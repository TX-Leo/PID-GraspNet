...
class GraspNetEval(GraspNet):
    ...
    **Input:**
    ...
    def eval_ann(self, scene_id,ann_id, dump_folder, TOP_K = 50, return_list = False,vis = False, max_width = 0.1):
        '''
        **Input:**
        ...
        **Output:**
        ...
        for ann_id in range(ann_id,ann_id + 1):
            ...
            print('\nMean Accuracy for scene:%04d ann:%04d = %.3f' % (scene_id, ann_id, 100.0 * np.mean(grasp_accuracy[:,:])), end='', flush=True)
            scene_accuracy.append(grasp_accuracy)
